...
describe('validateInternalKey middleware', () => {
  ...
  it('returns 403 when x-internal-key header is missing', () => {
    const req = { path: '/transactions', headers: {} };
    const res = mockRes();
    const next = jest.fn();
    validateInternalKey(req, res, next);
  ...
  it('returns 403 when x-internal-key is wrong', () => {
    const req = { path: '/transactions', headers: { 'x-internal-key': 'wrong' } };
    const res = mockRes();
    const next = jest.fn();
    validateInternalKey(req, res, next);
    expect(res.status).toHaveBeenCalledWith(403);
  ...
  it('calls next() when x-internal-key matches', () => {
    const req = { path: '/transactions', headers: { 'x-internal-key': MOCK_KEY } };
    const res = mockRes();
    const next = jest.fn();
    validateInternalKey(req, res, next);
    expect(next).toHaveBeenCalledTimes(1);
